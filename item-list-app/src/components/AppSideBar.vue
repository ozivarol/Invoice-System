<template>
    <aside class="bg-gray-700 w-[300px] h-screen">
      ...
      <div v-for="invoce in invoces" :key="invoce.id" class="odd:bg-gray-600 flex justify-between items-center p-2">
        <span>#{{ invoce.id.toString().slice(7, 11) }}</span>
        <span>{{invoce.contact.contact_name}}</span>
        <span>
          ...
          <button @click="editInvoce(invoce)" class="purple-button">
...
<script setup>
defineProps({invoces:Array,editInvoce:Function})
